...
#Where the application is actually ran

app = FastAPI(
    title="Glados test",
    description="Glados TTS Engine API",
    version="1.0.0",
)
...
glados_engine=None
try:
    glados_engine = tts_runner(False, True)
...
async def generate_audio(request: TTSRequest):
    if not glados_engine:
        raise httpsexceptions.HTTPException(status_code=503, detail="TTS Engine not available")
    try:
        audio_seg = glados_engine.speak(request.text, request.alpha)
        audio_bytes=BytesIO(audio_seg)
        return StreamingResponse(
            audio_bytes,
            media_type="audio/wav",
            headers={"Content-Disposition": "attachment; filename='glados_speech.wav'"}
        )
    except Exception as e:
        # Log the error and return a generic error message
        print(f"Inference error: {e}")
        raise HTTPException(status_code=500, detail="Audio generation failed during inference.")
